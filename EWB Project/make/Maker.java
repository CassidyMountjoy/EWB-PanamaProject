...
class Maker {
    ...
    void readFileAges(String fileInfoName) {
        ...
        name = "<Unknown>";
        try {
            Scanner inp = new Scanner(new java.io.File(fileInfoName));
            ...
            error("Near entry for error %s: %s", name, excp.getMessage());
        } catch (IOException excp) {
            error("Could not open file error %s", fileInfoName);
        }
    }
            ...
            while (inp.hasNextLine()) {
                ...
                if (parsed.matches()) {
                    ...
                    if (!parsed.group(2).isEmpty()) {
                        dependencies.addAll(
                                asList(SPACES.split(parsed.group(2))));
                    }
                ...
                if (target != null && parsed.matches()) {
                    ...
                    error("Erroneous input line error: '%s'", line);
                }
            }
            addRule(target, dependencies, commands);
        } catch (IOException excp) {
    ...
    class MakeTraversal extends DepthFirstTraversal {
         ...
         * In dependency graph call rebuild
         * on v0! because rebuild takes in the timing.
         * */
        MakeTraversal() {
            super(_depends);
        }
